...
//Invoke chaincode
function invoke(user, msg_id, timestamp, current_temperature, humid, cb) {
    ...
    var invokeRequest = {
        ...
        args: [msg_id, timestamp, current_temperature, humid]
    };
    ...
    tx.on('complete', function(results) {
        ...
        query(user, msg_id, function(err, results) {
            if (err) {
...
app.post('/process_invoke', function(req, res) {
    ...
    var msg_id = body.msg_id;
    var timestamp = body.timestamp;
    var current_temperature = body.current_temperature;
    var humid = body.humid;
    console.log("********* invoke input values: order_id:%s,package_id:%s,current_temperature:%s", msg_id, timestamp, current_temperature, humid);
    enroll(function(err, enrolleduser) {
        ...
        invoke(enrolleduser, msg_id, timestamp, current_temperature, humid, function(err, results) {
            if (err) {
                console.log('***********Unknown Error');
                return;
            }
            ...
            var strStatus = JSON.parse(results.result);
            console.log(strStatus.status);
            if (strStatus.status != "Valid") {
				console.log("Temp and humidity are more than predefined values by Blockchain consortium");
				console.log("smart contract conditions are statisfied and instruction are send to device");
                var options = {
                    hostname: '83c52072.ngrok.io',
                    port: null,
                    path: '/stopfan',
                    method: 'GET',
                    headers: {
                ...
                var reqs = http.request(options, function(respose) {
                    ...
                    respose.on('end', function() {
                        console.log("device is shutdown and the status is updated in consortium");
                    });
                });
                reqs.end();
            } else {
