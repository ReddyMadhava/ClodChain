        ...
        */
        ...
        func (t *trackShipmentTemp) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
                ...
                err := stub.CreateTable("detailedOrder", []*shim.ColumnDefinition{
                        &shim.ColumnDefinition{Name: "msg_id", Type: shim.ColumnDefinition_STRING, Key: true},
                        ...
                        &shim.ColumnDefinition{Name: "status_flag", Type: shim.ColumnDefinition_STRING, Key: false},
                })

                _, err = stub.InsertRow("detailedOrder", shim.Row{
                        Columns: []*shim.Column{
                                &shim.Column{Value: &shim.Column_String_{String_: "123"}},
                                &shim.Column{Value: &shim.Column_String_{String_: "12:00:11PM"}},
                                &shim.Column{Value: &shim.Column_String_{String_: "12"}},
                                &shim.Column{Value: &shim.Column_String_{String_: "50"}},
                                &shim.Column{Value: &shim.Column_String_{String_: "Valid"}}},
                })
        ...
        func (t *trackShipmentTemp) updateAsset(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
                ...
                current_temp, _ := strconv.Atoi(string(args[1]))
                humidity, _ := strconv.Atoi(string(args[3]))

                if len(args) != 4 {
                        return nil, errors.New("Incorrect number of arguments. Expecting 4")
                }

                fmt.Errorf("Function Invoked Current - [%d] max temp - [%d]", current_temp, humidity)

                if current_temp < 32 && humidity <= 90 {
                        ...
                        if len(row.Columns) == 0 {
                                ...
                                _, err1 = t.insertRow(stub, args, "detailedOrder", "Valid")
                                _, err1 = t.insertRow(stub, args, "uniqueOrder", "Valid")
                        ...
                        if len(row.Columns) == 0 {
                                ...
                                _, err1 = t.insertRow(stub, args, "uniqueOrder", "Invalid")
                        } else if row.Columns[12].GetString_() == "satisfied" {
                                _, err1 = t.updateRow(stub, "uniqueOrder", "Invalid", row)
                        }
                        if err1 != nil {
                                return nil, err1
                        }
        ...
        func (t *trackShipmentTemp) getCompleteOrderDetails(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
                ...
                if order_id != "" {
                        ...
                        for row := range rows {
                                msg_id := row.Columns[0].GetString_()
                                timestamp := row.Columns[1].GetString_()
                                current_temp := row.Columns[2].GetString_()
                                humidity := row.Columns[3].GetString_()
                                status := row.Columns[4].GetString_()
                                strRes := "{\"msg_id\": \"" + msg_id + "\"," + "\"timestamp\": \"" + timestamp + "\"," + "\"current_temp\": \"" + current_temp + "\"," + "\"humidity\": \"" + humidity + "\"," + "\"status\": \"" + status + "\"}"
        ...
        func (t *trackShipmentTemp) getUniqueOrderQuery(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
                ...
                if order_id != "" {
                        ...
                        order_details := "{\"msg_id\": \"" + msg_id + "\"," + "\"timestamp\": \"" + timestamp + "\"," + "\"current_temp\": \"" + current_temp + "\"," + "\"humidity\": \"" + humidity + "\"," + "\"status\": \"" + status + "\"}"
                        return []byte(order_details), nil

                } else {
                        return nil, errors.New("Excepting Msg Id")
                }
                return nil, nil
        }
